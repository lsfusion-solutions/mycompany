...
### Импорт из файла с помощью OpenAI

Если для выбранного типа поступления настроен промпт, в карточке поступления появляется действие импорта из файла с помощью OpenAI.
...
- в **«К оплате»** документ подтвержден для дальнейших действий (регистрация оплаты, печать — если используется). Соответствующее действие в карточке — **«Отметить к выполнению»**.
- в **«Оплачен»** поступление считается закрытым (действие **«Отметить как оплачен»**);
- в **«Отменён»** поступление исключается из учета (действие **«Отменить»**).

Также доступно действие **«Копировать»**, которое создаёт новое поступление в статусе «Черновик» с теми же шапкой и строками.
