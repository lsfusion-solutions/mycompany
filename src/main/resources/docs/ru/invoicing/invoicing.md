...
3. Переведите реализацию в статус **«К оплате»** (действие **«Отметить к выполнению»** в карточке; этого требует процесс).
...
![Раздел «Расчёты» в навигаторе с раскрытыми группами](images/invoicing-nav.webp)

Раздел «Расчёты» обычно содержит группы:

- **Операции** — [поступления](bills.md), [реализации](invoices.md), [входящие](incoming-payments.md) и [исходящие](outgoing-payments.md) платежи, а также связанные с ними документы корректировок.
- **Процессы** — вспомогательные обработки (зависят от конфигурации).
- **Отчётность** — отчёты по задолженности и [календарь платежей](debt-and-calendar.md).
- **Настройка** — параметры и справочники (форма **«Настройки»**, типы документов, [налоги](taxes.md), [условия оплаты](debt-and-calendar.md), банковские/кассовые счета).
...
#### [Корректировка](refunds-and-corrections.md)

Отдельный документ, исправляющий или сторнирующий ранее подтверждённое поступление либо реализацию. Корректировки поступлений поддерживают два режима — **замещение** и **сторнирование**; корректировки реализаций поддерживают только замещение.

#### [Возвратное поступление / Возвратная реализация](refunds-and-corrections.md)

Документы для оформления возвратов:

- **возвратное поступление** — это [поступление](bills.md) специального типа (на типе включён признак **«Возврат»**), которое реверсирует продажу со стороны поставщика;
- **возвратная реализация** — это [реализация](invoices.md) специального типа (на типе включён признак **«Возврат»**), которая реверсирует закупку со стороны покупателя.
...
- **«К оплате»** — документ подтвержден для дальнейших действий (печать, создание связанных документов, распределение оплат);
- **«Оплачен»** — документ полностью оплачен (в карточке поступления/реализации действие, которое переводит документ в этот статус, называется **«Отметить как оплачен»**); у самих платежей одноимённый финальный статус — **«Выполнен»**;
- **«Отменён»** — документ исключен из учета/процесса.
