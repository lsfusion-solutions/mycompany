...
![Раздел «Проекты» в навигаторе с раскрытыми группами](images/projects-nav.webp)
...
- **Отчётность** — точка расширения для отчётов по проектам. В базовой поставке папка пуста (и поэтому скрыта в меню): проектные разрезы добавляются в существующие отчёты соседних модулей (Продажи, Производство) — см. [Отчётность](reports.md).
...
**[Назначение](team-and-roles.md#назначения)** — привязка участника (сотрудника или команды) к проекту с указанием роли на проекте и периода участия. У задачи нет отдельной записи назначения — она ссылается на единственного исполнителя через своё поле «Назначена на».
