...
![Карточка расчётного листа](images/payslip-card.webp)
...
Ручные строки добавляются кнопкой **«Добавить»** (клавиша `Insert`) и удаляются действием **«Удалить»** у строки.

### Признаки «Пропустить» и «Спрятать»

Эти признаки относятся к **виду расчёта** (категории), а не к отдельной строке:

- **«Пропустить»** — строки этого вида расчёта не участвуют в расчёте итога «К выдаче».
- **«Спрятать»** — строки этого вида расчёта не показываются в таблице, но участвуют в расчёте (если вид расчёта не помечен также как «Пропустить»).

Подробное правило расчёта итога см. на странице [Как считается итог «К выдаче»](net-wage.md).

## Копирование расчётного листа

Действие **«Копировать»** создаёт новый расчётный лист на основе текущего, копируя **период**, **сотрудника**, **компанию**, **тип** и строки расчёта, введённые вручную. Номер присваивается заново; срок исполнения, привязка к зарплатной ведомости и валюта (устанавливается валютой по умолчанию) не переносятся. Начисления, сформированные по отметкам времени из проектов, не копируются; действие **«Сгенерировать»** доступно только в зарплатной ведомости, поэтому для их пересчёта привяжите новый лист к ведомости (поле **«Зарплатная ведомость»**) и выполните **«Сгенерировать»** там — либо введите строки вручную. После копирования проверьте период.
