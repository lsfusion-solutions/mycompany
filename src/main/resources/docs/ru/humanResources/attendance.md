...
- Список **«Регистрации времени»** находится в **«Кадры» → «Операции» → «Регистрации времени»**.
- **Мобильная регистрация времени** и **киоск регистрации времени** находятся в **«Кадры» → «Процессы»**.

![Список записей регистрации времени](images/attendances-list.webp)

## Что фиксируется

Для каждой записи регистрация времени хранит:

- **Сотрудник** — при мобильной или ручной регистрации автоматически устанавливается на текущего пользователя; запись киоска относится к сотруднику, распознанному по отсканированному бейджу.
- **Дата и время входа (Check In)** — обязательно.
- **Дата и время выхода (Check Out)** — необязательно до момента выхода.
- **Отработанные часы** — время между входом и выходом. В формах бэк-офиса показываются десятичным числом часов; в мобильном списке — в формате `ЧЧ:ММ`.
...
- крупные кнопки **«Старт»** (вход) / **«Стоп»** (выход);
...
- итоги **«Неделя»** и **«Месяц»** в блоке **«Итого»**. В итоги входят только **закрытые** записи (открытая запись учитывается после выхода); запись относится к неделе/месяцу по дате **входа**.
...
- Если **широта/долгота не получены**, кнопки **«Старт»**/**«Стоп»** отключены.
...
Когда сотрудник нажимает **«Старт»**:
...
Действие **«Стоп»** запрашивает подтверждение.
...
Диалог подтверждается автоматически через несколько секунд, либо сотрудник может нажать **«Отменить»**. Если сотрудник не распознан, на экране отображается сообщение **«Сотрудник не найден»**.

### Особенности киоска

- Киоск игнорирует повторное сканирование того же значения в течение примерно 10 секунд (чтобы избежать двойного сканирования).
...
- Если выключена, кнопки **«Старт»/«Стоп»** требуют валидную геолокацию.
...
- **Регистрации времени** — список записей с фильтрами:
  - **«Открыт»** (выход ещё не выполнен) — включён по умолчанию
  - **«Закрыта»** (выход выполнен) — выберите его, чтобы увидеть завершённые записи
...
- записанные даты/время входа и выхода (они проставляются сервером в момент отметки, а не берутся с часов устройства);
