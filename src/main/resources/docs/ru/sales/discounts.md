...
![Список скидок](images/discounts-list.webp)
...
![Карточка скидки с условиями](images/discount-card.webp)
...
- если у скидки заданы (включены) места хранения, она применяется только к строкам с одним из них; вложенные места хранения включённого места также считаются включёнными;
...
- общий объём предыдущих покупок больше порога (поле скидки **«Продано клиенту всего от»**);
- или объём покупок за прошлый календарный месяц больше порога (поле скидки **«Продано клиенту за прошедший месяц от»**).
...
- **«Продано в прошлом месяце»** в карточке покупателя — сумма продаж за предыдущий календарный месяц.

При расчёте берётся значение на момент перед текущей сессией (т.е. покупки в той же сессии не сдвигают порог).

### 7) По меткам контрагента

Скидку можно ограничить контрагентами с определёнными метками:

- если в скидке указаны метки, она применяется только к контрагентам, у которых есть одна из этих меток;
- если метки не указаны, ограничение не действует.
