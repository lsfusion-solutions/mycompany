...
![Список дисконтных карт](images/discount-cards.webp)

## Основные данные карты

У дисконтной карты есть:

- **номер** — идентификатор карты; он же является кодом, который сканируется на кассе;
...
Номер карты присваивается автоматически нумератором.
...
- **На кассе** — введите или отсканируйте номер карты в поле штрихкода. Владелец карты становится покупателем чека (см. [Касса и POS](pos.md)).
- **В реализации** — дисконтную карту можно выбрать в [реализации](../invoicing/invoices.md); при выборе подставляется покупатель, и система проверяет, что карта соответствует покупателю и не заблокирована.
