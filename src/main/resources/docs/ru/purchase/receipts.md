...
1. Проверяет, есть ли по заказу остаток к приёмке (поле **«Готово»** по строкам).
2. Ищет связанную приемку в статусе **«Готова»** или создаёт новую с типом, указанным в типе заказа.
3. Синхронизирует у этой приемки шапку (поставщик, плановая дата, место хранения) с заказом.
4. Добавляет/удаляет строки в соответствии с актуальным остатком; первоначальная потребность по строке приёмки фиксируется равной остатку «Готово».
5. Если в заказе не осталось строк с остатком — резервная приемка удаляется.

Эта приемка пересоздаётся/обновляется при изменениях `Готово`, поставщика, плановой даты, места хранения или номера заказа, а также при возврате заказа из «Заблокирован» в «Подтверждён».

Если приёмка превысит остаток по строке (попытка принять больше, чем заказано), система выдаст сообщение «Поступление превышает количество в заказе» и не сохранит документ.
...
В [типе заказа](settings.md) есть два независимых флага, влияющих на действие **«Заблокировать»**:

- **«Запретить блокировку при активных приемках»** — система не даст заблокировать заказ, пока у него есть резервная приемка в статусе «Готова». Если флаг выключен, такая приемка просто удаляется при блокировке.
- **«Запретить блокировку при неполной приемке»** — система не даст заблокировать заказ, если остаток `Готово > 0` (есть строки, которые ещё не приняты полностью).

При нарушении любого из этих правил выводится соответствующее сообщение, и заказ остаётся в статусе «Подтверждён».
