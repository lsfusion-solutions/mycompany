# Приемки по заказам поставщикам

Приемка фиксирует факт приема товара в место хранения и помогает контролировать исполнение заказа: сколько уже принято и сколько осталось.

## Где находится

Обычно работа с приемками доступна:

- из карточки **заказа поставщику** — в блоке связанных документов;
- в разделе складского контура (если он используется) — зависит от конфигурации.

## Связь с заказом поставщику

Приемка может формироваться на основании подтверждённого заказа. В этом случае:

- реквизиты приемки (поставщик, место хранения, плановая дата) обычно подставляются из заказа;
...
Как правило, приемки используются, когда включён складской контур.

Обычно приемка становится доступной после того, как:

1. Заказ переведён в статус **«Подтверждён»**.
2. В заказе указано **место хранения**.
...
Примечание: приемки обычно формируются **по товарным позициям**. Если в заказе есть услуги, для них приемка, как правило, не требуется.
...
Важно: приемка фиксирует **факт приема в место хранения** и используется для контроля исполнения заказа «сколько уже принято».
