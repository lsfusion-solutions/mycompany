...
3. Вставьте:
   - **Ключ** = Client ID из APS;
   - **Секрет** = Client Secret из APS.
4. Сохраните.
...
Пока флаг не установлен, вкладка **Autodesk** на формах номенклатуры / проекта / спецификации / производственного заказа скрыта.

## 4. Проверка

Быстрая проверка корректности подключения:

1. Откройте форму **integrations** и перейдите на вкладку **«Объекты»** со списком бакетов Autodesk.
2. Нажмите **Получить бакеты** — если учётные данные верны, ответ откроется в виде JSON-файла (на этом этапе список может быть пустым — это нормально).
3. Если возвращается **401 Unauthorized** — учётные данные неверны. Введите их заново.
4. Если возвращается другая ошибка — см. [Бакеты и модели — решение проблем](autodesk-buckets-and-models.md#решение-проблем).

Теперь вы готовы [создать бакет и загрузить модель](autodesk-buckets-and-models.md). После того как модель будет преобразована и привязана, конечные пользователи увидят её на формах [номенклатуры / проекта / спецификации / производственного заказа](autodesk-viewer.md).
