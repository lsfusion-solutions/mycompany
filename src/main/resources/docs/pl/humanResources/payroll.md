# Płace: naliczanie i wypłata

Sekcja „Płace: naliczanie i wypłata” służy do naliczania przychodów i potrąceń pracowników za okres oraz (jeśli potrzeba) do rejestrowania płatności.

## Strony w tej sekcji

- [Partia odcinków płacowych](payroll-batch.md)
- [Odcinek płacowy](payslip.md)
- [Jak obliczana jest suma „Wynagrodzenie netto”](net-wage.md)
- [Wypłata na podstawie przepracowanego czasu (Projekty)](payroll-time-entries.md)
- [Wypłata wynagrodzenia i kontrola płatności](payroll-payments.md)

## Główne obiekty

#### Partia odcinków płacowych

Dokument używany do **zbiorczego generowania odcinków płacowych** dla pracowników dla wybranego **przedsiębiorstwa** i **okresu**.

Szczegóły: [Partia odcinków płacowych](payroll-batch.md).

#### Odcinek płacowy

Dokument naliczenia pracownika za okres. Zawiera linie przychodów/potrąceń oraz sumę **„Wynagrodzenie netto”**.

Szczegóły: [Odcinek płacowy](payslip.md).

## Typowy scenariusz „naliczenie płac i przygotowanie wypłaty”

1. Otwórz **„Kadry” → „Operacje” → „Partii odcinków płacowych”**.
2. Utwórz partię i wskaż przedsiębiorstwo oraz okres.
3. Uruchom **„Wygeneruj”**.
4. Otwórz odcinki płacowe pracowników i sprawdź linie naliczeń oraz sumę **„Wynagrodzenie netto”**.
5. Jeśli w organizacji używa się rejestracji płatności — zarejestruj płatność.

Zalecenia i reguły:

- jak partia wybiera pracowników i co robi „Wygeneruj” — [Partia odcinków płacowych](payroll-batch.md);
- jak czytać odcinek płacowy i weryfikować dane źródłowe — [Odcinek płacowy](payslip.md);
- jak obliczane jest „Wynagrodzenie netto” — [Jak obliczana jest suma „Wynagrodzenie netto”](net-wage.md).
