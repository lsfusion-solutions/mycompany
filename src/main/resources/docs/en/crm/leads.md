...
- **“Comments”** — a comment feed for the lead. Comments support mentioning users with **@**; attached files and key changes are shown together with the comments. The sales person and mentioned users can receive email notifications about new comments (enabled in the user profile). You can edit and delete (with a confirmation) only your own comments — the icons appear on hover; an edited comment gets an **“Edited”** mark (hover over it to see who changed it and when). A user with the **Manage all comments** flag on the employee card can edit and delete any comments. The **Comment editing period (min)** setting (**Master data → Settings**) limits how long the author can change a comment (does not apply to moderators; empty — no limit).
