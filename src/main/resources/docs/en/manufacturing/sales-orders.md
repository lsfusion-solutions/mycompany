...
The system supports a scenario where **manufacturing orders are created based on sales order lines**.

This is useful when you sell items that must be produced for a specific order.

## Conditions for creation

Manufacturing orders are created from a sales order when all conditions are met:

1. The **sales order type** has a **manufacturing order type** specified.
2. The sales order has the **confirmed** status.
3. The sales order contains lines where:
   - a **Bill of Materials** is specified;
   - a manufacturing order has not been created yet.

If any condition is not met, the creation button may not be shown, or the action will create nothing.

## How to select a Bill of Materials in a sales order line

A sales order line has a Bill of Materials field.

Rules:

- the Bill of Materials must match the line item (otherwise the system will not allow saving the line);
- if the item has a default Bill of Materials, it may be selected automatically.
...
3. For each line where a Bill of Materials is specified and no manufacturing order exists yet, a new manufacturing order is created.

## What is filled in the created manufacturing order

A manufacturing order is created **per sales order line**.
...
- the system automatically creates manufacturing orders for order lines.

## Change synchronization

If automatic creation is enabled, changes in sales order lines are synchronized with the manufacturing order until it is started:
