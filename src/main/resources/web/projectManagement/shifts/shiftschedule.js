function shiftSchedule() {
    ...
    // Paint the employee's base64 photo (server-provided) as the avatar background. The photo is a
    // data: URL already in memory (nothing to fetch), so paint it SYNCHRONOUSLY. Drawing the initials
    // placeholder first and swapping to the photo on a later img.onload would make every avatar flash
    // to initials and back whenever the grid rebuilds (e.g. after a reassign) — that's the "flicker".
    // We still preload an Image purely to detect a bad/corrupt data URL and only then fall back to
    // initials. __avUrl stamps this call's token so a stale onerror can't clobber a newer render.
